...
class RunFoundationTests(unittest.TestCase):
    ...
    def test_all_dry_run_describes_ordinary_delivery_without_running_children(self):
        ...
        self.assertEqual(plan["recent_non_pinned"], 5)
        self.assertEqual(plan["max_comments_per_video"], 0)
        self.assertEqual(plan["privacy_mode"], "hash")
        self.assertEqual(plan["works_login_wait"], 30.0)
        self.assertEqual(plan["comment_login_wait"], 60.0)
        ...
        resume_args = build_parser().parse_args([
            "all", "--creator", "https://www.douyin.com/user/test", "--recent", "5",
            "--profile-dir", "profile", "--out", "delivery", "--resume", "--dry-run",
        ])
        resume_plan = all_plan(resume_args)
        self.assertTrue(resume_plan["resume"])
        self.assertIn("works stage skipped", resume_plan["browser_session"])
    ...
    def test_all_runs_three_stages_into_one_delivery_tree(self):
        with workspace_temp() as temp:
            ...
            def fake_browser_stage_runner(
                works_args, comments_args, trace_path, skip_works=False
            ):
                browser_calls.append((works_args, comments_args, trace_path, skip_works))
            ...
            self.assertEqual(browser_calls[0][1].login_wait, 60.0)
            self.assertFalse(browser_calls[0][3])
    ...
    def test_resume_requires_matching_complete_works_manifest(self):
        with workspace_temp() as temp:
            ...
            works_json.write_text(
                json.dumps([{"source_url": "https://www.douyin.com/video/12345678901"}]),
                encoding="utf-8",
            )
            manifest.write_text(
                json.dumps(
            ...
            validate_resume_works(
                manifest, works_json, "https://www.douyin.com/user/test", 5
            )
            with self.assertRaisesRegex(FoundationError, "recent count"):
                validate_resume_works(
    ...
    def test_all_resume_skips_complete_works_stage_and_builds_delivery(self):
        with workspace_temp() as temp:
            ...
            delivery = temp / "delivery"
            preview = temp / "preview"
            works_out = delivery / "data" / "作品采集"
            works_out.mkdir(parents=True)
            (works_out / "works.json").write_text(
                json.dumps([{"source_url": "https://www.douyin.com/video/12345678901"}]),
                encoding="utf-8",
            )
            (works_out / "download_manifest.json").write_text(
                json.dumps(
                    {
                        "status": "complete",
                        "creator": "https://www.douyin.com/user/test",
            ...
            self.assertEqual(len(browser_calls), 1)
            self.assertTrue(browser_calls[0][3])
    ...
    def test_shared_browser_resume_skips_works_runner(self):
        with workspace_temp() as temp:
            calls = []
            ...
            result = run_shared_browser_stages(
                argparse.Namespace(profile_dir=str(temp / "profile"), chrome_path=""),
                argparse.Namespace(),
                temp / "trace.jsonl",
                playwright_factory=fake_playwright_factory,
                works_runner=lambda *_args, **_kwargs: calls.append("works"),
                comments_runner=lambda *_args, **_kwargs: calls.append("comments") or 0,
                chrome_finder=lambda _value: "chrome.exe",
                skip_works=True,
            )
            self.assertEqual(result, (0, 0))
            self.assertNotIn("works", calls)
            self.assertIn("comments", calls)
            events = [
                json.loads(line)
                for line in (temp / "trace.jsonl").read_text(encoding="utf-8").splitlines()
            ]
            self.assertIn("works_stage_skipped", [event["event"] for event in events])
