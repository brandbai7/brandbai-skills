...
明确单作品任务打开真实 `/video/` 或 `/note/` 页面后，还会在当前作品主体中观察公开可见的小黄车入口。采集器只识别播放器内具有商品控件结构的元素，不根据视频文案、字幕、话题或普通文字推测商品；同一轮播存在多个控件时，只接受当前进入视口且位于最上层的一个。随后只读取该控件渲染出来的展示名和页面直接提供的 HTTP(S) 链接；不点击入口、不进入商品详情，不读取搜索页、达人主页或推荐区的商品卡片。观察不到时写 `not_observed`，只代表本次页面可见范围。

## 显式挂车商品路线

以上小黄车段描述的是默认被动观察。只有用户请求关联商品资料时，才使用 `--commerce-detail`：暂停目标作品，从目标作品当前入口打开新的商品面板，稳定后读取公开资料。预先打开的未知商品面板、多个候选面板或变化中的作品不能复用。只采集结构明确的规格区域和公开服务承诺，排除个性化收货区域。

用户另行要求商品评价时增加 `--product-reviews`，在同一个页面定位“商品评价”，使用随 Skill 分发的评价采集器滚动当前列表，逐批保存并显示进度。评价与作品评论是独立阶段，商品阶段暂停／中断时不继续自动操作其他页面；细节见 [商品评价与挂车联合采集](product-reviews.md)。

读取中的验证、访问限制、页面隐藏、商品或筛选变化都应停止，不自动重试绕过。商品图片下载拒绝非图片响应；未取得资源保持缺失，不能把登录网页命名为 JPG。
