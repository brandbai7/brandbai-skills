...
3. 作品任务可从主页置顶＋最近 N 条、搜索页当前观察、插件作品清单或明确作品 URL 获取范围；评论任务优先读取该任务生成的 `works.json`，确保范围一致。
...
搜索结果或任意多选优先读取插件导出的 `作品清单.xlsx`。Excel 只提供选择与基础字段时，作品阶段在同一可见 Chrome 上依次打开所选作品补齐正常页面返回的素材元数据；第一页保留登录等待，后续页面不重复套用完整登录等待。
...
作品下载也接受带 `modal_id` 的抖音链接；输出仍规范化为真实 `/video/` 或 `/note/` 路径。
...
- `all` 模式分别使用作品主页的 `--login-wait` 和逐作品评论面的 `--comment-login-wait`；后者默认 60 秒，避免把首次登录等待重复施加到每条作品。
- 预计超过宿主调用时限时使用 `scripts/run_long_job.py start`，随后只用 `status` 轮询同一个任务。任务状态、PID、stdout、stderr 和最终退出码保存在交付目录之外的 job 目录。
- 被强制中断且作品侧已经 complete 时，使用相同范围增加 `all --resume`。续跑会校验作品 manifest，跳过作品阶段；已有 `done_reason=exhausted` 的一级评论作品也会跳过导航。
- 不要根据系统全部 `chrome.exe` 数量判断残留进程；只能关闭明确属于专用资料夹的可见采集窗口或已核实的对应进程，绝不能批量结束用户的普通 Chrome。
