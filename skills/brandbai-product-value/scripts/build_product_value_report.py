...
def public_text(value: Any, empty: str = "未提供") -> str:
    ...
    text = re.sub(r"列入\s+(待复核|待确认)(?:项)?", r"列入\1项", text)
    text = re.sub(r"冲突在\s+留档", "冲突已留档", text)
    text = re.sub(r"相关规格在\s+留档\s*(?:待|等待)?实物复核", "相关规格已留档，等待实物复核", text)
    text = re.sub(r"[；;]\s*详(?:见)?\s*$", "", text)
