...
完成两遍核对后，先写 `source_observation.jsonl`，再写来源和事实。禁止沿用旧交付的页序、标题或语义映射；文件名相似、数字相邻或上一次分析结果都不能替代本次逐文件核对。每条本地来源必须同时绑定清单中的 `source_file_id` 和核对记录中的 `observation_id`，`locator` 必须包含清单里的原始相对路径；只有直接 URL 来源可以不绑定本地文件。
...
3. 只有全部图片通过 `审计卡哈希一致 + 正序初检 + 逆序复核` 后才能建立来源账本；来源标题必须与该文件的核对标题一致，来源同时绑定 `source_file_id` 和 `observation_id`；
4. 穷举当前可确认事实，隔离跨 SKU、历史版本、动态字段和冲突；
5. 为每个准备进入 P0/P1/P2 的价值建立独立 FABE 记录，完整写出 Feature、Advantage、Benefit、Evidence、参照系、用户语言、推导状态和边界；参数不能直接当用户利益；标为页面直接支持时，Evidence 至少包含一条 Feature 的直接事实，不能用无关检测页支撑另一种体验利益；
6. 分离商品身份、一级识别锚、P0 候选、P1、P2 与 DYN；
7. 建立完整 P0 候选池，将品牌指定方向纳入候选但不自动判胜；
8. 分别判断战略价值潜力和当前执行成熟度，不机械合并总分；
9. 形成推荐战略 P0 及状态、当前执行主轴、P1、P2、暂缓价值和表达边界，并另写一段不含内部 ID 的 `public_rationale` 供普通版使用；
10. 写清当前能证明什么、不能证明什么和下一步验证问题；“不能证明”必须置于明确否定标题下，不能让否定对象单独显示成正面结论；
11. 记录资料缺口、完成状态和下游可用范围后停止。
...
只有退出码为 `0` 才能作为正式交付。校验会检查商品与 SKU、审计卡是否真实内嵌清单中对应原图、审计卡哈希、正序初检与逆序复核、批量时间回填、页面截图的小字证据跨字段绕过、FABE 直接相关性、来源和事实引用、稳定 ID、价值适用范围、P0 决策、完成状态、资料缺口、普通版占位符和下游边界。
