...
来源清单保留原始文件名、相对路径、大小和 SHA-256。生成后不得为了视觉顺序重命名或重新编号；需要重做时新建交付目录。完成索引后，必须逐个打开清单中的准确文件路径，先写 `source_observation.jsonl`，再写来源和事实。禁止沿用旧交付的页序、标题或语义映射；文件名相似、数字相邻或上一次分析结果都不能替代本次逐文件核对。每条本地来源必须同时绑定清单中的 `source_file_id` 和核对记录中的 `observation_id`，`locator` 必须包含清单里的原始相对路径；只有直接 URL 来源可以不绑定本地文件。
...
2. 先生成不可覆盖的真实文件清单；按 `source_file_id` 逐个打开准确文件，立即记录原相对路径、可见标题、可见文字摘录、页面内容类型和核对时间；图片必须实际打开原图，禁止只读文件名、缩略图顺序、旧报告或 OCR 汇总；
3. 只有完成逐文件核对后才能建立来源账本；来源标题必须与该文件的核对标题一致，来源同时绑定 `source_file_id` 和 `observation_id`；
...
页面截图和详情页图片可以证明其清楚展示的大字主张、机构、检测项目和结果，但一律不能把报告编号、日期、批次、证书编号、检测方法等小字精确字段标为已核验，证据细节可信度最高为 `medium`。只有报告原件/PDF可定位文本或官方验证页，才允许 `exact_fields_verified=true`，并必须填写 `verification_locator`。不得把“反复蒸晒”扩写成“九蒸九晒”，也不得把“入口温和”扩写成“无刺激”。
...
只有退出码为 `0` 才能作为正式交付。校验会检查商品与 SKU、真实来源文件与逐文件视觉核对绑定、页面截图的小字证据边界、FABE 直接相关性、来源和事实引用、稳定 ID、价值适用范围、P0 决策、完成状态、资料缺口、普通版占位符和下游边界。
