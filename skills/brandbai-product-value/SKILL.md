...
6. 为每个准备进入 P0/P1/P2 或进入 P0 候选池的价值建立独立 FABE 记录，完整写出 Feature、Advantage、Benefit、Evidence、参照系、用户语言、推导状态和边界；参数不能直接当用户利益；标为页面直接支持时，Evidence 至少包含一条 Feature 的直接事实，不能用无关检测页支撑另一种体验利益；没有页面明确对比、竞品页或行业资料时，不虚构“普通产品、无法确认工艺的产品、多配料加工食品、单一食用方式产品、散装或大包装、无检测引用产品”等替代对象；只有引用 `U` 用户原声或研究事实时，才能把用户旧习惯写成参照系，否则改写为页面内具体对比或明确标注的内生任务假设；
...
`01` 回答“这是什么、为什么值得选、凭什么信”，固定展示用户问题、FABE 价值证据链、当前可调用的 P0/P1/P2、为什么这样分层和条件式下游接口；暂缓或禁止调用的价值不进入正式价值清单，但如果它属于 P0 候选，仍必须出现在“核心价值候选比较”中并说明未入选原因。`02` 说明资料来源、成熟度、冲突、未知、停止边界和下一步补充。普通版隐藏 `PV-/SF-/OBS-/CLM-/SRC-/F-/FABE-/V-` 等内部资产 ID、内部英文字段和技术状态，完整底稿放入 `data/`，供后续 Skill 或审计继续使用。交付根目录不得携带事后修正脚本或可执行文件。
...
只有退出码为 `0` 才能作为正式交付。校验会检查商品与 SKU 冲突、规格算术和跨页包数冲突、疑似 OCR 残片、冲突规格是否继续进入锚点/FABE/价值/P0、交付根目录、审计卡是否真实内嵌清单中对应原图、审计卡哈希、核验事件是否晚于 `created_at`、正序初检与逆序复核的真实时间顺序、固定节奏与晚于账本文件的可疑时间、第三遍是否全部完成后才开始第四遍、最终账本和报告是否在 `updated_at` 后继续修改、动态活动年份和时区、关键警示、事实原文引用、数字、口感词与使用限制是否能回到原文、页面截图的小字证据跨字段绕过、FABE 直接相关性与比较依据、无来源产品替代对象、无用户证据的旧习惯参照和用户排名词、绝对化利益、交易叠加证据、来源和事实引用、稳定 ID、价值适用范围、完整 P0 候选池、候选展示、完成状态、资料缺口、普通版占位符、客户残句和下游边界。
