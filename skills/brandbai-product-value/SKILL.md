...
只有退出码为 `0` 才能作为正式交付。校验会检查商品与 SKU 冲突、交付根目录、审计卡是否真实内嵌清单中对应原图、审计卡哈希、核验事件是否晚于 `created_at`、正序初检与逆序复核的真实时间顺序、固定节奏与晚于账本文件的可疑时间、第三遍是否全部完成后才开始第四遍、最终账本和报告是否在 `updated_at` 后继续修改、动态活动年份和时区、关键警示、事实原文引用、数字、口感词与使用限制是否能回到原文、页面截图的小字证据跨字段绕过、FABE 直接相关性与比较依据、无用户证据的旧习惯参照、绝对化利益、交易叠加证据、来源和事实引用、稳定 ID、价值适用范围、完整 P0 候选池、候选展示、完成状态、资料缺口、普通版占位符和下游边界。
