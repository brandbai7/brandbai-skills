...
6. 为每个准备进入 P0/P1/P2 或进入 P0 候选池的价值建立独立 FABE 记录，完整写出 Feature、Advantage、Benefit、Evidence、参照系、用户语言、推导状态和边界；参数不能直接当用户利益；Advantage 必须写出具体任务差异，禁止用“本品（基于页面内对比信息）”等占位语补齐；若资料不足以形成可核对优势，明确写“当前资料不足以形成可核对的相对优势，A层暂不成立”，并将 `derivation_status` 标为 `to_validate`；标为页面直接支持时，Evidence 至少包含一条 Feature 的直接事实，不能用无关检测页支撑另一种体验利益；没有页面明确对比、竞品页或行业资料时，不虚构“普通产品、无法确认工艺的产品、多配料加工食品、单一食用方式产品、散装或大包装、无检测引用产品”等替代对象；只有引用 `U` 用户原声或研究事实时，才能把用户旧习惯写成参照系，否则改写为页面内具体对比或明确标注的内生任务假设；
...
只有退出码为 `0` 才能作为正式交付。校验会检查商品与 SKU 冲突、规格算术和跨页包数冲突、疑似 OCR 残片、冲突规格是否继续进入锚点/FABE/价值/P0、交付根目录、审计卡是否真实内嵌清单中对应原图、审计卡哈希、核验事件是否晚于 `created_at`、正序初检与逆序复核的真实时间顺序、固定节奏与晚于账本文件的可疑时间、第三遍是否全部完成后才开始第四遍、最终账本和报告是否在 `updated_at` 后继续修改、动态活动年份和时区、关键警示、复合事实逐字段回链、事实原文引用、数字、口感词与使用限制是否能回到原文、页面截图的小字证据跨字段绕过、FABE 直接相关性与比较依据、占位式 Advantage、无来源产品替代对象、无用户证据的旧习惯参照和用户排名词、绝对化利益、交易叠加证据、来源和事实引用、稳定 ID、价值适用范围、完整 P0 候选池、候选展示、完成状态、资料缺口、结构化账本与普通版中的空括号、缺少结论对象的残句、其他占位符和下游边界。
