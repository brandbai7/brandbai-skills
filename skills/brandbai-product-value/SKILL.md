...
6. 为每个准备进入 P0/P1/P2 或进入 P0 候选池的价值建立独立 FABE 记录，完整写出 Feature、Advantage、Benefit、Evidence、参照系、用户语言、推导状态和边界；参数不能直接当用户利益；标为页面直接支持时，Evidence 至少包含一条 Feature 的直接事实，不能用无关检测页支撑另一种体验利益；没有页面明确对比、竞品页或行业资料时，不写“相对普通、相对添加多种、相对未明示”等替代性比较；
...
只有退出码为 `0` 才能作为正式交付。校验会检查商品与 SKU、交付根目录、审计卡是否真实内嵌清单中对应原图、审计卡哈希、正序初检与逆序复核的真实时间顺序、含文字来源的第三遍原文摘录与第四遍复核、关键警示、事实原文引用、数字、口感词与使用限制是否能回到原文、页面截图的小字证据跨字段绕过、FABE 直接相关性与比较依据、交易叠加证据、来源和事实引用、稳定 ID、价值适用范围、完整 P0 候选池、候选展示、完成状态、资料缺口、普通版占位符和下游边界。
