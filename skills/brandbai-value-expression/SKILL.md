...
9. 到手量可以写已确认规格；“整周、几天、一个月”等使用周期只有在上游存在频次或周期事实时才能写，不得由包数自行换算；
10. 选择最多5个核心VIS进入普通版，至少包含推荐P0的一个感知资产；可沟通价值不超过5个时，每个价值各选1个主价值VIS，不用同一价值的重复卡挤掉其他价值；一级识别锚不得进入核心卡；
11. 如有验证条件，最多提出3个单变量建议任务；分别写清对照版、测试版、唯一变量、指标获取方式和判断规则。若唯一变量是证据画面，两版字幕、商品、时长、声音等保持一致；若验证单包构成，两版都必须来自当前SKU真实单包，只改变连续性等单一证明方式；未登记样本量和统计方法时不写“显著性”，只记录方向；
12. 记录资料缺口、完成状态、失效条件和后续内容组装接口后停止。
